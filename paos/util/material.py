...
class Material:
    ...
    def __init__(self, Tambient=-218.0, wl=None, materials=None):
        """
        Parameters
        ----------
        Tambient: scalar
        ...
        if wl is None:
            self.wl = np.linspace(0.5, 8.0, 1024)
            logger.debug('Using default wavelength grid')
        else:
            self.wl = wl
    ...
    def nmat0(self, name, wl0, wl1=None):
        """
        Given the name of an optical glass, a minimum and a maximum wavelength, returns the absolute and scaled
        relative index of refraction at the central wavelength.
        ...
        name: str
            name of the optical glass
        wl0: float
            minimum wavelength in micron
        wl1: float
            maximum wavelength in micron

        Returns
        -------
        out: tuple
            returns a tuple with the values the glass index of refraction at the central wavelength:
            one for the absolute index and the other for the index relative to air.
        """
        if wl0 is None:
            logger.error('User should provide a value for the minimum wavelength')

        if wl1 is not None:
            wlc = 0.5 * (wl0 + wl1)
            idx = np.argmin(np.abs(self.wl - wlc))
            nmat_abs, nmat = self.nmat(name)
            nmat_abs0, nmat0 = nmat_abs[idx], nmat[idx]

        else:
            idx = np.argmin(np.abs(self.wl - wl0))
            nmat_abs, nmat = self.nmat(name)
            nmat_abs0, nmat0 = nmat_abs[idx], nmat[idx]

        return nmat_abs0, nmat0
