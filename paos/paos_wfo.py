...
class WFO(object):
    ...
    def lens(self, lens_fl):
        ...
        if   propagator == 'II': lens_phase = 1.0/lens_fl
        elif propagator == 'IO': lens_phase = 1/lens_fl + Cima
        elif propagator == 'OI': lens_phase = 1.0/lens_fl - Cobj
        elif propagator == 'OO': lens_phase = 1.0/lens_fl - Cobj + Cima
    ...
    def propagate(self, dz):
        """
        Wavefront propagator. Selects the appropriate propagation primitive and applies the 
        wf propagation
    ...
    def zernikes(self, index, Z, ordering, normalize, radius, offset=0.0, origin='x'):
        """
        Add a WFE represented by a Zernike expation
        ...
        origin: string
            angles measured counter-clockwise positive from x axis by default (origin='x'). Set origin='y' for angles measured clockwise-positive from the y-axis.
        Returns
        -------
        out: the WFE 
        """
